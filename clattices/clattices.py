...
def parseArgs():
	...
	parser.add_argument('-o', '--output_file', default='CoincidenceLattices.dat', nargs=1, help="output file for combinations table (default: CoincidenceLattices.dat file)")
	...
	parser.add_argument('-s', '--angles_step', type=float, nargs=1, default=0.1, help="step for the investigation of angles (default: 0.1)")
...
def main():
	...
	angles = np.linspace (args.angles[0], args.angles[1], (args.angles[1]-args.angles[0])/args.angles_step + 1)
	...
	if args.self_combinations:
		for i in range (len(crystals)):
			for j in range (i, len(crystals)):
				combinations.append (Combination.Combination([crystals[i], crystals[j]], angles, [args.N, args.tolerance]))
	else:
		for i in range (len(crystals)):
	...
	for c in combinations:
		...
		c.findSolutions()
		supercellList.append (c.findMinimumArea())
